    ...
    for (i in 1:n_respostas) {
      ...
      if (!is.null(model)) {
        ...
        if (n >= 5) {
          ...
          cat("--------------- Testes de Significância ---------------\n")
          cat("Não é possível calcular as estatísticas de significância (Erro Padrão, t-valor, p-valor) para os parâmetros do segmento linear (b0 e b1). Causa: O número de pontos de dose (N) é inferior a 5. O ajuste linear requer pelo menos 3 pontos no segmento linear (N_linear >= 3) para ter graus de liberdade residuais.\n")
        }
      } else {
