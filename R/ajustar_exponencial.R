...
#'
#' Implementa o ajuste do modelo de regressão exponencial: Y = a * exp(b * X).
...
"
  ...
  if (n < 3) {
    stop("O modelo exponencial requer no mínimo 3 pontos de dose.")
  }
  ...
  for (i in 1:n_respostas) {
    ...
    # log(Y) = log(a) + b * X
    linear_fit <- lm(log(Y) ~ X, data = data)
    ...
    tryCatch(
      {
        ...
        p <- ggplot2::ggplot(
          ...
          ggplot2::labs(
            title = sprintf("Ajuste Exponencial - %s", nome_resposta),
            x = "Dose",
            y = nome_resposta,
            color = "Tipo",
        ...
        if (verbose) {
          ...
          cat(sprintf("Modelo: Exponencial (nls)\n"))
          cat(sprintf(
      ...
      error = function(e) {
        ...
        if (verbose) {
          ...
          cat(
            "❌ Não foi possível ajustar o Modelo Exponencial (NLS falhou)\n\n"
          )
        }
      }
    )
