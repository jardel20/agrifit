...
#'
#' Implementa o ajuste do modelo de regressão quadrática: Y = b0 + b1 * X + b2 * X^2.
#'
...
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1"
  ...
  if (n < 4) stop("O modelo quadrático requer no mínimo 4 pontos de dose.")
  ...
  for (i in 1:n_respostas) {
    ...
    data <- data.frame(X = dose, Y = resposta, X2 = dose^2)
    
    # Ajuste do Modelo
    model_fit <- lm(Y ~ X + X2, data = data)
    ...
    resultado_df <- data.frame(
      ...
      parametro = c("b0_intercepto", "b1_linear", "b2_quadratico", "R2", "RMSE", "AIC", "BIC",
    ...
    equacao_str <- sprintf("%s: Ŷ = %.4f + %.4fX + %.4fX²", nome_resposta, b0, b1, b2)
