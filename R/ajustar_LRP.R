...
ajustar_LRP <- function(dose, ...,
  ...
  for (i in 1:n_respostas) {
    ...
    for (k in 3:(n - 1)) {
      ...
      # Cálculo CORRIGIDO de AIC, BIC, RMSE
      RMSE <- sqrt(SSE / n)
      K <- 3 # b0, b1, b2

      # Log-Verossimilhança CORRETA (normalidade, σ² estimado)
      sigma2 <- SSE / (n - K)
      logL <- -(n / 2) * log(2 * pi * sigma2) - (n / 2)

      AIC_val <- -2 * logL + 2 * K
      BIC_val <- -2 * logL + log(n) * K

      # Testes + ICs (com proteção df_res)
      lm_summary <- summary(lm_fit)
      lm_coefs <- lm_summary$coefficients
      df_res <- lm_summary$df[2]

      if (df_res > 0 && nrow(lm_coefs) >= 2) {
        p_b0 <- lm_coefs[1, 4]
        p_b1 <- lm_coefs[2, 4]
        t_b0 <- lm_coefs[1, 3]
        t_b1 <- lm_coefs[2, 3]
