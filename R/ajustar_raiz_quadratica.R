...
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1"
  ...
  if (n < 4) {
    stop("O modelo raiz quadrático requer no mínimo 4 pontos de dose.")
  }
  ...
  for (i in 1:n_respostas) {
    ...
    Y_pred <- predict(
      model_fit,
      newdata = data.frame(X = dose_pred, X_sqrt = sqrt(dose_pred))
    )
    df_plot <- data.frame(
    ...
    p <- ggplot2::ggplot(df_plot, ggplot2::aes(x = dose, y = Y, color = tipo)) +
      ...
      ggplot2::labs(
        title = sprintf("Ajuste Raiz Quadrática - %s", nome_resposta),
        x = "Dose",
        y = nome_resposta,
        color = "Tipo",
